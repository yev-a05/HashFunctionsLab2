...
def generate_precomputation_table(K: int, L: int, r: bytes):
    """Генерує таблицю передобчислень (Алгоритм 1)."""
    table = {}
    for _ in range(K):
    ...
    print("\nЗгенерована таблиця передобчислень (перші 10 записів):")
...
def find_preimage(table, L, r, h_value):
    """Пошук прообразу для h (Алгоритм 2)."""
    y = h_value
    attempts = 0
    for _ in range(L):
        if y in table:
...
def experiment(K, L, N):
    ...
    for _ in range(N):
        ...
        found_x, attempts = find_preimage(table, L, r, h_value)
